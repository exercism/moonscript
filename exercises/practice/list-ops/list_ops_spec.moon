...
describe 'list-ops:', ->
  describe 'append entries to a list and return the new list:', ->
    ...
    pending 'list to empty list', ->
      result = ListOps.append {}, {1, 2, 3, 4}
      expected = {1, 2, 3, 4}
      assert.are.same expected, result

    pending 'empty list to list', ->
      result = ListOps.append {1, 2, 3, 4}, {}
      expected = {1, 2, 3, 4}
      assert.are.same expected, result

    pending 'non-empty lists', ->
  ...
  describe 'concatenate a list of lists:', ->
    ...
    pending 'list of lists', ->
      result = ListOps.concat {{1, 2}, {3}, {}, {4, 5, 6}}
      expected = {1, 2, 3, 4, 5, 6}
      assert.are.same expected, result

    pending 'list of nested lists', ->
      result = ListOps.concat {{{1}, {2}}, {{3}}, {{}}, {{4, 5, 6}}}
      expected = {{1}, {2}, {3}, {}, {4, 5, 6}}
      assert.are.same expected, result
  ...
  describe 'folds (reduces) the given list from the left with a function:', ->
    ...
    pending 'direction independent function applied to non-empty list', ->
      result = ListOps.foldl {1, 2, 3, 4}, 5, (acc, el) -> el + acc
      expected = 15
      assert.are.equal expected, result

    pending 'direction dependent function applied to non-empty list', ->
