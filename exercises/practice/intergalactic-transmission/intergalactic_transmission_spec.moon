...
describe 'intergalactic-transmission:', ->
  ...
  describe 'decode received messages:', ->
    ...
    pending 'zero message', ->
      result = decodeMessage {'0x00', '0x00'}
      expected = {'0x00'}
      assert.are.same expected, result

    pending '0x0300 is decoded to 0x02', ->
      result = decodeMessage {'0x03', '0x00'}
      expected = {'0x02'}
      assert.are.same expected, result

    pending '0x0581 is decoded to 0x05', ->
      result = decodeMessage {'0x05', '0x81'}
      expected = {'0x05'}
      assert.are.same expected, result

    pending '0x2881 is decoded to 0x29', ->
      result = decodeMessage {'0x28', '0x81'}
      expected = {'0x29'}
      assert.are.same expected, result

    pending 'first byte has wrong parity', ->
      f = -> decodeMessage {'0x07', '0x00'}
      assert.has.error f, 'wrong parity'

    pending 'second byte has wrong parity', ->
      f = -> decodeMessage {'0x03', '0x68'}
      assert.has.error f, 'wrong parity'

    pending '0xcf4b00 is decoded to 0xce94', ->
      result = decodeMessage {'0xcf', '0x4b', '0x00'}
      expected = {'0xce', '0x94'}
      assert.are.same expected, result

    pending '0xe2566500 is decoded to 0xe2ad90', ->
    ...
    pending 'last byte has wrong parity', ->
      f = -> decodeMessage {
        '0x47',
        '0xb8',
        '0x99',
    ...
    pending 'wrong parity on 16th byte', ->
      f = -> decodeMessage {
        '0x44',
        '0xbd',
        '0x18',
